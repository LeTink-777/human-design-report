...
export async function sendResultEmail({
  ...
  const { data, error } = await resend().emails.send({
    ...
    subject,
    html: `
      <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background: #FDFAF6; color: #2A2622;">
        <h1 style="color: #C4896B; font-size: 24px; margin-bottom: 20px;">${escapeHtml(siteName)}</h1>
