 ...
 * /api/checkout, а данные рождения читаются из metadata этого платежа, а не из
 * тела запроса. Без проверки эндпоинт отдавал бы платный отчёт любому, кто
 * пришлёт дату рождения, а доверие к телу запроса позволило бы покупателю
 * базового тарифа получить полную версию.
 */
export async function POST(request: Request) {
  ...
  if (!input) {
    return NextResponse.json({ error: "В заказе нет данных рождения." }, { status: 422 });
  }

  const pdfBuffer = await generatePDF({
    title: "Ваш отчёт Human Design",
    userName: input.name || "Дорогой клиент",
  ...
  return new NextResponse(new Uint8Array(pdfBuffer), {
    headers: {
      ...
      "Content-Disposition": 'attachment; filename="human-design.pdf"',
